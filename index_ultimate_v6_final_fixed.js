/**
 * 💀 المُتحكِّم V6 - النسخة النهائية الكبرى (إصلاح Manus) 💀
 * تم إصلاح الرسالة الترحيبية + نظام النقاط + منطق الإرسال للضحية
...
const CONFIG = {
    ...
    ATTACK_DELAY: 1500, // زيادة التأخير قليلاً لتجنب الحظر
    INITIAL_CREDITS: 10,
        ...
        try {
            ...
            if (!user.authenticated) {
                if (text === CONFIG.PASSWORD) {
                    ...
                    const welcomeMsg = `✨ 🔱 مـرحـبـاً فـي بـوت الـمـتـحـكـم 🔱 ✨\n\n` +
                    ...
                    if (!userState.has(jid + '_w')) {
                        await sock.sendMessage(jid, { text: '💀 الـرجـاء إدخـال كـلـمـة الـسـر لـلـوصـول:' });
            ...
            if (text.toLowerCase() === 'اوامر' || text.toLowerCase() === 'start' || text === 'مرحبًا') {
                ...
                const menu = `✨ 💀 قـائـمـة الـسـيـطـرة 💀 ✨\n\n` +
                           `💰 رصـيـدك: 【 ${user.credits} 】\n\n` +
                           `1️⃣ - إرسـال رسـالـة خـاصـة\n` +
                           `2️⃣ - مـعـلـومـات الـمـطـور\n` +
                           `3️⃣ - قـصـف الأعداء (تدمير)\n` +
                           (jid === ADMIN_JID ? `8️⃣ - هـجـوم نـووي ☢️\n` : "") +
                           `\n📧 الـدعم: ${CONFIG.OFFICIAL_EMAIL}`;
            ...
            if (state?.step === 'menu') {
                if (text === '1') {
                    if (user.credits > 0 || jid === ADMIN_JID) {
                    ...
                    if (user.credits > 0 || jid === ADMIN_JID) {
                        ...
                        await sock.sendMessage(jid, { text: '💀 أدخل رقم الضحية المراد تدميره:' });
                    } else {
                        await sock.sendMessage(jid, { text: '❌ نـفـد رصـيـدك! تـواصـل مـع الـمـطـور لـلـشـحـن.' });
                    }
