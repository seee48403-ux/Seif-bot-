/**
 * 💀 المُتحكِّم V6 - النسخة النهائية الكبرى (Final Grand Edition) 💀
 * تم دمج الشتائم الجديدة + نظام النقاط + دقة الإرسال للضحية
 */
...
// --- المصفوفة الضخمة (دمج القديم والجديد) ---
const insults = [
    // الشتائم الأصلية
    ...
    // الشتائم الجديدة من الملف المرفق
    ...
    "انت بالمنظر دا تروح تموت نفسك انتحر ي ابن العبيطة", "انما تسد وتمد لا هيداس عليك زي منا م دايس عليك كدة",
        ...
        try {
            ...
            if (!user.authenticated) {
                if (text === CONFIG.PASSWORD) {
                    ...
                    await db.save(CONFIG.DB_FILE, users);
                    await sock.sendMessage(jid, { text: `✅ تم التفعيل! رصيدك: ${user.credits} نقطة.` });
                } else {
            ...
            if (text.toLowerCase() === 'اوامر' || text.toLowerCase() === 'start') {
                ...
                const menu = `💀 قائمة السيطرة 💀\n💰 رصيدك: ${user.credits}\n\n1-إرسال رسالة\n2-المطور\n3-قصف الأعداء\n` + (jid === ADMIN_JID ? `8-نووي` : "");
            ...
            if (state?.step === 'menu') {
                if (text === '1') {
                    if (user.credits > 0 || jid === ADMIN_JID) {
                        ...
                        await sock.sendMessage(jid, { text: '❌ رصيدك 0.' });
                    }
                } else if (text === '2') {
                    await sock.sendMessage(jid, { text: 'المطور: ' + CONFIG.ADMIN_NUMBER });
                } else if (text === '3') {
                    if (user.credits > 0 || jid === ADMIN_JID) {
                    ...
                    await sock.sendMessage(jid, { text: 'رقم النووي؟' });
                }
                return;
            }
            ...
            if (state?.step === 'get_a') {
                ...
                if (jid !== ADMIN_JID) user.credits--;
                await db.save(CONFIG.DB_FILE, users);
                await sock.sendMessage(jid, { text: `🚀 بدأ القصف! الرصيد: ${user.credits}` });
                (async () => {
            ...
            if (state?.step === 'get_p') {
                userState.set(jid, { step: 'get_m', target: text.replace(/[^\d]/g, '') + '@s.whatsapp.net' });
            ...
            if (state?.step === 'get_m') {
                ...
                await sock.sendMessage(jid, { text: `✅ تم! الرصيد: ${user.credits}` });
                userState.set(jid, { step: 'menu' });
                return;
            }
            ...
            if (state?.step === 'nuke' && jid === ADMIN_JID) {
                let target = text.replace(/[^\d]/g, '') + '@s.whatsapp.net';
                await sock.sendMessage(jid, { text: '☢️ نووي فعال!' });
                (async () => {
                    while(true) {
                        for (const insult of insults) {
                            try { 
